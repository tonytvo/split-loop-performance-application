...
import org.splitloopperf.config.EmbeddedKafka;
import org.splitloopperf.config.KafkaSseConsumer;
import org.splitloopperf.config.KafkaSseProducer;
...
@WithMockUser
@EmbeddedKafka
class SplitLoopPerformanceKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/split-loop-performance-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/split-loop-performance-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/split-loop-performance-kafka/unregister"));
                return;
            }
        }
